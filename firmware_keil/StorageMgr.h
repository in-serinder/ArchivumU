...
BLOCK_ARR:5*16(键值对数量)byte 初始化后整个区块写0xff 写入和读取逻辑 [写入
高位一字节通过0x0 表示第一片 0x1 表示第二片] 后面低四位为24c64片内地址 0x0000 ~
0x1FFF BLOCK_VER: 1byte 校验位 0x01
